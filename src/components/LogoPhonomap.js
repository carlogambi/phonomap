...
const Layer = ({src, alt, style}) => (<img src={src} style={style} alt={alt} />)
...
const LogoPhonomap = ({style, showInfo, deviation, getDeviation}) => {
    ...
    const updateElementPosition = () => {
        ...
        if(elementRef){
            const { x,  y, height, width} = elementRef.current.getBoundingClientRect()
            logoCenterPositions = {
    ...
    const updateMousePosition = () => {
        if(deviceDetector() === 'desktop'){
            mouseTraker((currentMousePosition) => 
                (mousePosition !== currentMousePosition) 
                && 
                setMousePosition(currentMousePosition)
            )
        }else{
            touchTraker((currentMousePosition) => 
                (mousePosition !== currentMousePosition) 
                && 
                setMousePosition(currentMousePosition)
            )
    ...
    logoStyle = style ? {...logoStyle, ...style}: {...logoStyle, ...defaultlogoStyle}

    return (
            <div id='logo-phonomap' style={logoStyle} ref={elementRef} onClick={(e) => logoClickedEvent.trigger()}>
                {showInfo && (<div style={infoStyle}>
                    <h3 style={{margin: '0px'}}>LOGO ANIMATION</h3>
                    mouse x{mousePosition.x} y{mousePosition.y}<br/>
                    element x{centerPosition.x} y{centerPosition.y}<br/>
                ...
                <Layer 
                    ...
                    style={{
                        ...ShadowLayerStyle(false, 
                            incrementDeviation(
                                deviation.shadowLogo
                            )), //reference -> 2
                            zIndex: 13}} 
                    src={logoText}
                />
